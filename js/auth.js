...
//****************************
            ...
            cognitoUser.getSession(function(err, session) {
                ...
                // Get Id
                // var idenParams = {
                //     IdentityPoolId: "ap-southeast-1:4f7d85d7-109f-49a4-8c35-d528467762b8",
                //     AccountId: "364721626014",
                //     Logins:{ // use access token provided by UserPool
                //         "cognito-idp.ap-southeast-1.amazonaws.com/ap-southeast-1_bj23F51JC": session.getIdToken().getJwtToken()
                //     }
                // }
                var idenParams = {
                    IdentityPoolId: ip,
                ...
                idenParams["Logins"] = {};
                idenParams["Logins"][cap] = session.getIdToken().getJwtToken();
                ...
                cognitoidentity.getId(idenParams, function(err, data) {
                    if (err) console.log(err, err.stack); // an error occurred
                        ...
                        credParams["Logins"] = {};
                        credParams["Logins"][cap] = session.getIdToken().getJwtToken();

                        cognitoidentity.getCredentialsForIdentity(credParams, function(err, data) {
                            if (err) console.log(err, err.stack); // an error occurred
                            else { // successful response
                                window.location.replace('pages/ihydrogarden/locations.html');
                                ...
                                // params["Key"] = "pages/locations.html"
                                // s3.getObject(params, function(err, data) {
                                //     if (err) console.log(err, err.stack);
                                //     else {
