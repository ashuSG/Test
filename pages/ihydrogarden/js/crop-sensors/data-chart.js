...
$(function() {
  ...
  createDirectWaterUsageContentDiv(localStorage.getItem("current_project"), localStorage.getItem("current_device"), 1);
  createDirectAccumWaterUsageContentDiv(localStorage.getItem("current_project"), localStorage.getItem("current_device"), 0);

  if ($("#report-type").val() != "individual-sensors") {
    $(".row.data-names-bar").hide()
  }
...
function startChartPoller(pollRate) {
  // getChartData(pollRate)
}
...
// Show or hide sensor checklists based on report type selected
$("#report-type").click(function() {
  if ($("#report-type").val() != "individual-sensors") {
    $(".row.data-names-bar").hide()
  } else {
    $(".row.data-names-bar").show()
  }
})

$("#apply-chart-setting-btn").click(function() {
  startDate = getEpochTime($("#datetimepicker1 > input:text").val());
  endDate = getEpochTime($("#datetimepicker2 > input:text").val());
  ...
  // Call different query functions based on mode
  var type = $("#report-type").val()
  if (type == "individual-sensors") {
    // Chart based on individual sensors    
    var checkedZoneNames = getCheckedValues("zone-names-list");
    console.log(checkedZoneNames);

    var checkedIndSNames = getCheckedValues("indirect-sen-names-list");
    console.log(checkedIndSNames);

    var checkedDirectSNames = getCheckedValues("direct-sensor-names-list");
    console.log(checkedDirectSNames); 

    if (checkedZoneNames.length == 0 && checkedIndSNames.length == 0 && checkedDirectSNames.length == 0) {
      alert("No sensor data selected");
    } else {
      $("#no-data-selected-msg").hide();
      queryIndivChart(checkedZoneNames, checkedIndSNames, checkedDirectSNames, startDate, endDate);
      $("#graphdiv2").css("background-color", "white");
    }
  } else {
    // Chart based on water usage
    var mode = type.charAt(0).toUpperCase();
    $("#no-data-selected-msg").hide();
    console.log("Calling queryWUsageChart");
    queryWUsageChart(startDate, endDate, mode);
    $("#graphdiv2").css("background-color", "white");
  }
});

function getEpochTime(stringDateTime){
    return new Date(stringDateTime).getTime()/1000
}
...
function getDevices() {
  ...
  var url = apiBaseUrl + "/Devices/get-devices";
...
function createDirectWaterUsageContentDiv(projectName, deviceName, zoneNumber){
  tagName = projectName + "/" + deviceName + "/WUsage/" + zoneNumber;
  tagNameDisp = projectName.charAt(0).toUpperCase() + projectName.substr(1).toLowerCase() + "/" + deviceName + "/Water-Usage/" + zoneNumber;

  let divHtml = '<div class="form-check">' +
  '<input type="checkbox" class="form-check-input chart-names" ' +
  'id="dSWUsa-' + zoneNumber + '" value="' + tagName + '">' +
  '<label class="form-check-label" for="dSWUsa-' + zoneNumber + '" >' + tagNameDisp +
  '</div>';

  $("#direct-sensor-names-list").append(divHtml);
}

function createDirectAccumWaterUsageContentDiv(projectName, deviceName, zoneNumber){
  tagName = projectName + "/" + deviceName + "/AccumWUsage/" + zoneNumber;
  tagNameDisp = projectName.charAt(0).toUpperCase() + projectName.substr(1).toLowerCase() + "/" + deviceName + "/Accum-Water-Usage/" + zoneNumber;

  let divHtml = '<div class="form-check">' +
  '<input type="checkbox" class="form-check-input chart-names" ' +
  'id="dSAccWU-' + zoneNumber + '" value="' + tagName + '">' +
  '<label class="form-check-label" for="dSAccWU-' + zoneNumber + '" >' + tagNameDisp +
  '</div>';
