...
function validZoneSelection() {
  ...
  $(".row-irrigation.valve").each(function() {
    $(this).find(".valve-dropdown").each(function() {
      ...
      if (zoneId != NaN) {
        console.log("zones[zoneId]:", zones[zoneId]);
        if (zones[zoneId]) {
          isValid = false;
        }
        zones[zoneId] = true;
 ...
 * @returns boolean 
 */
function validIrrigationDuration() {
  totalTime = getTotalIrrigationTime();
  startTimes = getStartTimeList()
 ...
 */
function getTotalIrrigationTime() {
...
function getScheduleTimings() {
  ...
  irrigationTime = getTotalIrrigationTime();
